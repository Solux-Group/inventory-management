  ...
  const showRows = () => {
    ...
    if (page === totalPagesFiltered && totalRowsFiltered % rowsPerPage !== 0) {
      if (totalRowsFiltered === totalRows) {
        return `${start + 1} - ${start + left} sur ${totalRowsFiltered} data`;
      }

      return `${start + 1} - ${
        start + left
      } sur ${totalRowsFiltered} data (total ${totalRows} data)`;
    } else {
      // if total rows filtered === 0
      if (totalRowsFiltered === 0) {
        return `${0} - ${0} sur ${totalRowsFiltered} data (total ${totalRows} data)`;
      } else if (totalRowsFiltered === totalRows) {
        // if total rows === 0
        if (totalRows === 0) {
          return `${0} - ${0} sur ${totalRowsFiltered} data`;
        }

        return `${start + 1} - ${
          start + rowsPerPage
        } sur ${totalRowsFiltered} data`;
      }
